@java
  Feature: My first Java code

    @java1
    Scenario: Hello World
      Given I say Hello World
      When I print "Hello World!"

    @java2
    Scenario: Concatenation
      Given I work with strings

    @java3
    Scenario: String Method
      Given I act on "cucumber" and "CuCUMBER"
      Given I act on "men in black" and "Men in Black"
      Given I compare between "my job" and "my JOB"

    @java4
    Scenario: Number exercises
      Given I act on numbers

    @java5
    Scenario: Boolean exercises
      Given I demostrate boolean datatype
      Given my favorite color is "blue"

    @java6
    Scenario: Conditions exercise
      Given I print url for "google" page
      Given I print url for "bing" page

    @java7
    Scenario: Conditions - switch
      Given I print url for "google" using switch
      Given I print url for "ebay" using switch

    @java8
    Scenario: Array exercise
      Given I operate on arrays

    @java9
    Scenario: For Loop
      Given I work with arrays

    @java10
    Scenario: Print reverse string
      Given I revert a "Java" string

    @java11
    Scenario: Print Day of week
      Given I print day of week for number 1






